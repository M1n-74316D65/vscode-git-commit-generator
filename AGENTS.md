...
VS Code extension (`m1n.vscode-llm-api-git-commit-generator`, v1.3.0) that generates Git commit messages from repository changes using VS Code's built-in Language Model API (`vscode.lm`, e.g. GitHub Copilot) — no external HTTP calls, no API keys. Supports 15 commit styles, optional Gitmoji prefixes, seven commit-message languages, EN/ES UI localization, and smart body generation.

## Architecture & Data Flow

Entry: `src/extension.ts` exports `activate`/`deactivate`. `activate()` initializes the Output Channel logger, awaits legacy setting cleanup, subscribes to language-model availability changes, checks git availability, and registers commands (`registerCommands`, `registerConfigCommands`).
...
2. `GitManager.findGitRepositories()` + `resolveRepositoryRoot()` — repo discovery via `vscode.git` extension API (`getAPI(1)`), fallback `git rev-parse --show-toplevel` per workspace folder; active-editor containment chooses the deepest match, otherwise multi-root workspaces show a repository picker.
...
4. `GitManager.getDiff(scope, excludePatterns)` — shells out via `promisify(execFile)` (`git diff --staged` vs `git diff HEAD`, plus bounded-concurrency `git diff --no-index` sections for untracked files; unborn repositories diff all current files against `/dev/null`; 8MB maxBuffer, 30s timeout). Sections whose `diff --git a/x b/y` path matches `gitCommitGenerator.excludeFiles` globs are dropped (`src/glob.ts`, zero-dep matcher supporting `**`/`*`/`?`); stats are computed from the filtered diff content. Returns `undefined` on empty diff, throws on git failure.
...
6. `LLMManager.generateCommitMessage()` — model selection with 3-strategy fallback (preferred id → family → any) and a 5-minute static model cache; `compressContext` (hard pre-token diff bound, then iterative `model.countTokens` vs `maxInputTokens − 2000` headroom, drop recent commits, reduce diff at file/hunk/line boundaries ~20%/step, else `PromptTooLargeError`; shows `diffTooLarge` only when diff content shrank); `buildPrompt` (i18n system prompt + per-style rules + explicit untrusted-data boundary); `sendRequestWithRetry` (3 retries, linear backoff, retries only retryable `LanguageModelError`s, reports streaming char count via progress).
...
State: static mutable fields on manager classes (model cache and generation lock) + VS Code workspace configuration `gitCommitGenerator.*`. No DI container, no external state store.

## Key Directories

- `src/` — all extension source (entry, commands, git, llm, config, notifications, types, i18n, glob matcher)
...
- Managers are static classes (`GitManager`, `LLMManager`, `ConfigManager`, `LogManager`, `NotificationManager`) imported directly; only `GitManager` is instantiated per-repo (`new GitManager(root)`).
